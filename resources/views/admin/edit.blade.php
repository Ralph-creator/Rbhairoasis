...
<body class="bg-pink-50 min-h-screen">
    <header class="bg-pink-500 text-white p-4 flex justify-between items-center">
        ...
        <a href="{{ route('admin.dashboard') }}" class="bg-white text-pink-500 px-4 py-2 rounded hover:bg-pink-100">Back to Dashboard</a>
    </header>

    <main class="max-w-2xl mx-auto py-10">
        <h2 class="text-2xl font-semibold text-center mb-6">Edit Product</h2>

        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded-xl p-6">
            @csrf
            @method('PATCH')
            ...
            <div class="mb-4">
                ...
                <p class="text-sm text-gray-600 mt-1">Current: <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="h-24 mt-2"></p>
            </div>

            <div class="flex justify-between items-center">
                <button type="submit" class="bg-pink-500 text-white px-4 py-2 rounded hover:bg-pink-600">Update Product</button>
                <a href="{{ route('admin.dashboard') }}" class="text-pink-500">Cancel</a>
            </div>
        </form>
    </main>
</body>
